...
const ManageJobs = () => {
  ...
  return jobs ? jobs.length === 0 ? (
    <div className='flex items-center justify-center h-[70vh]'>
      <p className='text-xl sm:text-2xl'>No Jobs Availabe or Posted!</p>
    </div>) : (
    <div className='container p-4 max-w-5xl'>
              ...
              jobs.map((job, index) => (
                <tr className='text-gray-700' key={index}>
                  ...
                  <td className='py-2 px-4 border-b border-gray-300'>
                    <input className='scale-125 ml-4' onChange={() => changeJobVisibility(job._id)}
      ...
      <div className='mt-4 flex justify-end'>
        <button onClick={() => navigate('/dashboard/add-job')}
          className='uppercase bg-black text-white py-2 px-4 rounded cursor-pointer'>add new job</button>
      </div>
    </div>
  ) : <Loading />
}
